import six.moves.urllib.request as u
import bz2
import os
import pandas as pd
...
def save_preprocessing_images1(data_path, dir_save, w, fil, verbose = 0):
    #w = 16 # Window size (images of the size 2w*2w)
    data_path = str(data_path)
    dir_save = str(dir_save); w = int(w)
    objects = pd.read_csv(data_path)
    ob = 1
    filename = os.path.join(dir_save,fil)
    print('starting ......')
    for index, values in objects.iterrows():
        #try:
        rowc = int(round(values[[x for x in objects.columns if x.startswith('rowc_')]].mean()))
        ...
        del band_arrays
        # dir_save = '../data/images/galaxies1/all/'
        np.save(os.path.join(dir_save, str(values['class'])+'.'+str(int(values['objid']))+'_'+str(float(values['redshift']))), object_tensor)
        del object_tensor
        #except :
         #   print('incorrect object:')
    print('------------------------ dowloading ok ------------------------')
...
def save_all_model(model, file):
    model.save(file)
    print('model is saved')
def load_all_model(file):
...
def get_train_test_valid_data_galaxy1(dir_img, test_size = 0.21, valid_size = 0.4, size_all = None, size = 32):
    X, Y = get_all_in_folder1(dir_img, size_all, size)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    X_Test, X_Valid, Y_Test, Y_Valid = train_test_split(X_Test,Y_Test, test_size = valid_size)
...
def get_train_test1(file_csv, dir_img, test_size = 0.5,size_all = None):
    X, Y = get_all_in_folder1(dir_img, size_all)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    return X_Train, Y_Train, X_Test, Y_Test
...
def get_train_test_valid_data_galaxy2(dir_img, test_size = 0.21, valid_size = 0.4, size_all = None, size = 32):
    X, Y = get_all_in_folder2(dir_img, size_all, size)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    X_Test, X_Valid, Y_Test, Y_Valid = train_test_split(X_Test,Y_Test, test_size = valid_size)
...
def get_train_test2(file_csv, dir_img, test_size = 0.5,size_all = None):
    X, Y = get_all_in_folder2(dir_img, size_all)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    return X_Train, Y_Train, X_Test, Y_Test
...
def get_train_test_valid_data_galaxy3(dir_img, test_size = 0.21, valid_size = 0.4, size_all = None, size = 32):
    X, Y = get_all_in_folder3(dir_img, size_all, size)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    X_Test, X_Valid, Y_Test, Y_Valid = train_test_split(X_Test,Y_Test, test_size = valid_size)
...
def get_train_test3(file_csv, dir_img, test_size = 0.5,size_all = None):
    X, Y = get_all_in_folder3(dir_img, size_all)
    X_Train, X_Test, Y_Train, Y_Test =  train_test_split(X,Y, test_size = test_size)
    del X
    del Y
    return X_Train, Y_Train, X_Test, Y_Test
...
def get_all_in_folder1(images_folder, size_all = None, size = 32):
    caract='_'
    files = g.glob(images_folder)
    toStop = size_all != None
    try:
    ...
    for name in files:
        try:
            fileName, fileExtension = os.path.splitext(os.path.basename(name))
            na = fileName.split('_')[0]
            #if clas=='ALL' or na==clas:
            val = np.load(name)
            if not val.shape == (size,size,5):
                raise Exception('','')
            #val = getcolor(val)
            data.append(val)
            red = fileName.split(caract)[1]
            redshifts.append(float(red))
            if toStop and comp == size_all:
                break
            comp = comp + 1
        except:
            nbr = nbr + 1
...
def get_all_in_folder2(images_folder, size_all = None, size = 32):
    caract='_'
    files = g.glob(images_folder)
    toStop = size_all != None
    try:
...
def get_all_in_folder3(images_folder, size_all = None, size = 32):
    caract='_'
    files = g.glob(images_folder)
    toStop = size_all != None
    try:
...
# ---------------------------- new  -----------------------------
"""
def plot_result1(y_true, y_pred, x_name = "True redshift", y_name = "Predicted redshift",  title = "Fitness between predicted redshift and true redshift"):
    mi = min(y_true) 
    ma = max(y_true)
    a1, a2 = d(mi, ma) 
    b1, b2 = d1(mi, ma)
    ...
    pl.plot(y_true,y_pred, 'bo')
    pl.xlabel(x_name)
    pl.ylabel(y_name)
    pl.legend([y_name+' = '+x_name,y_name+' = '+x_name+' - 0.15', y_name+' = ' + x_name+' + 0.15', 'Redshift fitness'], loc='upper left')
    pl.show()  
"""
def save_pr_hist(predict, true_value,  history, pred = 'predict.npy', tr = "true.npy", hist = 'history.npy'):
    np.save(tr, true_value)
    np.save(pred, predict)
    np.save(hist, np.array([history.history['loss'], history.history['val_loss']]))

"""
def load_and_plot_fitness(pred = 'predict.npy', tr = "true.npy"):
    plot_result1(np.load(tr), np.load(pred))

def load_and_plot_history(hist = 'history.npy', ymax=None):
    tab = np.load(hist)
    pl.figure(figsize=(15, 7))
    pl.plot(tab[0])
    pl.plot(tab[1])
...
def rmse_loss_keras1(y_true, y_pred):
    diff = keras.backend.square((y_pred - y_true))*1000
    return keras.backend.sqrt(keras.backend.mean(diff))
